...
class Arbitrator:
    ...
    def __init__(self, data_model: DataModel):
        ...
        self.width: int = data_model.cols
        self.height: int = data_model.rows

        # Initialize the coverage map
        if self.height > 0 and self.width > 0:
            self.coverage_map: list[list[int]] = [[0 for _ in range(self.width)] for _ in range(self.height)]
        else:
            raise ValueError(f"Invalid grid size: height={self.height}, width={self.width}. Both must be > 0.")

    def turn_score(self, balloons: list[Vector3], debug: bool = False) -> int:
        cov = self.coverage_radius ** 2
        '''
        ...
        def is_covered(r, c, u, v) -> bool:
            ...
            '''
            return (r - u) ** 2 + columndist(c, v, self.width) ** 2 <= cov
